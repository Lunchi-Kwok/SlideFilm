...
class BinaryFocalLoss(nn.Module):
    """
    Binary focal loss for binary segmentation.
    Expected:
    ...
    def forward(self, logits, target):
        ...
        prob = prob.clamp(min=1e-7, max=1 - 1e-7)  # numerical stability

        ce1 = target * torch.log(prob)
        ce2 = (1 - target) * torch.log(1 - prob)

        focal_weight_1 = (1 - prob) ** self.gamma
        focal_weight_2 = prob ** self.gamma

        loss = - self.alpha * focal_weight_1 * ce1 \
               - (1 - self.alpha) * focal_weight_2 * ce2
...
class BinaryFocalDiceLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, lambda_dice=1.0):
        super().__init__()
        self.focal = BinaryFocalLoss(alpha=alpha, gamma=gamma, reduction="mean")
        self.dice = BinaryDiceLoss_()
        self.lambda_dice = lambda_dice
    ...
    for batch_ in Dataloaders:
        ...
        for tile_batch in tile_loader:
            ...
            with autocast(dtype=torch.float16):
                logits = decoder(fmap, slide_vec_b, skip_fmaps) if skip_fmaps is not None else decoder(fmap,
                                                                                                       slide_vec_b)
